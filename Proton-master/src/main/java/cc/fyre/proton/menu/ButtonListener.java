    ...
    public void onButtonPress(InventoryClickEvent event) {
        ...
        if (openMenu != null) {
            ...
            if (openMenu.getButtons().containsKey(event.getSlot())) {
                ...
                player.setMetadata("CLICKED_BUTTON", new FixedMetadataValue(Proton.getInstance(), player.getUniqueId().toString()));
                ...
                if (event.isCancelled()) {
                    Proton.getInstance().getServer().getScheduler().runTaskLater(Proton.getInstance(), player::updateInventory, 1L);
                }

                player.removeMetadata("CLICKED_BUTTON", Proton.getInstance());
