...
import org.cavepvp.entity.menu.npc.menu.NPCEquipmentMenu
import org.cavepvp.entity.type.npc.NPC
import org.cavepvp.entity.util.ItemBuilder
import java.util.*

class NPCEquipmentButton(private val npc: NPC) : Button() {
    ...
    override fun getButtonItem(player: Player): ItemStack {
        return ItemBuilder.of(Material.DIAMOND_HELMET)
            .name("${ChatColor.GOLD}${ChatColor.BOLD}Equipment")
            .build()
    }

    override fun clicked(player: Player, slot: Int, clickType: ClickType) {
        NPCEquipmentMenu(this.npc).openMenu(player)
    }
