...
class CraftMetaSkull extends CraftMetaItem implements SkullMeta {
    ...
    void applyToItem(final NBTTagCompound tag) { // Spigot - make final
        ...
        if (hasOwner()) {
            ...
            final String lowercase = CraftMetaSkull.this.profile.getName().toLowerCase();
            final GameProfile profile = TileEntitySkull.skinCache.asMap().get(lowercase);
            ...
            ThreadingManager.queueHeadConversion(() -> {
                ...
                final GameProfile finalProfile = net.minecraft.server.TileEntitySkull.skinCache.getUnchecked(lowercase);
                ...
                if ( finalProfile != null )
                {
                    MinecraftServer.getServer().processQueue.add( new Runnable()
                    {
                        ...
                        {
                            NBTTagCompound owner1 = new NBTTagCompound();
                            GameProfileSerializer.serialize(owner1, finalProfile );
                            tag.set( SKULL_OWNER.NBT, owner1);
                        }
                    } );
                }
