            ...
            if (true || list != null && !list.isEmpty()) { // CraftBukkit - Call event even if no effects to apply
                ...
                if (list1 != null) { // CraftBukkit - Run code even if there are no entities around
                    ...
                    if (affected.size() == 1) {
                        ...
                        final LivingEntity first = affected.entrySet().stream().findFirst().orElse(null).getKey();

                        if (first != null) {

                            final EntityLiving craft = ((CraftLivingEntity) first).getHandle();

                            if (craft.fireTicks > 0
                                    && craft.isInLava
                                    && craft.lastDamageSource != null
                                    && (craft.lastDamageSource instanceof EntityDamageSourceIndirect || craft.lastDamageSource instanceof EntityDamageSource && craft.lastDamageSource.getEntity() instanceof EntityPlayer)
                                    && craft.noDamageTicks >= 5
                            ) {
                                return;
                            }
                        }
