            ...
            if (d0 >= (double) (-range) && d0 <= (double) range && d1 >= (double) (-range) && d1 <= (double) range) {
                if (!this.trackedPlayers.contains(entityplayer) && (this.d(entityplayer) || this.tracker.attachedToPlayer)) {
                    ...
                    if (trackerInstanceOf && SpigotConfig.onlyCustomTab) {
                        ...
                        final EntityPlayer target = (EntityPlayer) tracker;

                        entityplayer.playerConnection.sendPacket(PacketPlayOutPlayerInfo.removePlayer(target));

                        if (!target.hasEffect(MobEffectList.INVISIBILITY)) {
                            entityplayer.playerConnection.sendPacket(PacketPlayOutPlayerInfo.addPlayer(target));
                        }

                        if (!entityplayer.hasEffect(MobEffectList.INVISIBILITY)) {
                            entityplayer.playerConnection.sendPacket(PacketPlayOutPlayerInfo.addPlayer(entityplayer));
                        }

//                            entityplayer.playerConnection.sendPacket(PacketPlayOutPlayerInfo.updateDisplayName((EntityPlayer) tracker ));
//                            entityplayer.playerConnection.sendPacket(PacketPlayOutPlayerInfo.updateDisplayName(entityplayer));
                    }
