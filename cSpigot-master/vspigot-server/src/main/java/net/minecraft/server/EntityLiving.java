...
public abstract class EntityLiving extends Entity {
    ...
    public int directionalKBCount = 0;
...
//                        && damagesource instanceof EntityDamageSourceIndirect && ((EntityDamageSourceIndirect) damagesource).getProximateDamageSource() instanceof EntityFishingHook;
                ...
                if (((!(shouldIgnoreArrow))) && this.noDamageTicks > maxNoDamageTicks / 2) {
                    ...
                    this.activatedTick = MinecraftServer.currentTick + this.maxNoDamageTicks * 2; // Kohi
                }
    ...
    public void a(DamageSource damageSource, float f, double d0, double d1) {
        ...
        if (this.random.nextDouble() >= this.getAttributeInstance(GenericAttributes.c).getValue()) {
            ...
            final Knockback profile = (this.getKnockback() == null) ? SpigotConfig.defaultKnockback : this.getKnockback();

            this.motX /= profile.getFriction();
            this.motY /= profile.getFriction();
            this.motZ /= profile.getFriction();

            double motX = d0 / magnitude * profile.getHorizontal();
            double motZ = d1 / magnitude * profile.getHorizontal();

            if (damageSource instanceof EntityDamageSource && damageSource.getEntity() instanceof EntityPlayer) {

                final EntityPlayer damager = (EntityPlayer) damageSource.getEntity();
                double rot = (this.yaw - 90) % 360.0;

                if (rot < 0) {
                    rot += 360.0;
                }

                double rot2 = (damager.yaw - 90) % 360.0;

                if (rot2 < 0) {
                    rot2 += 360.0;
                }

                final double minRange = rot - 100.0F;
                final double maxRange = rot + 100.0F;

                if (rot2 >= minRange && rot2 <= maxRange) {

                    if (this.directionalKBCount >= 1) {
                        motX /= 1.7;
                        motZ /= 1.7;
                    }

                    this.directionalKBCount++;
                } else {
                    this.directionalKBCount = 0;
                }

            }

            if (damageSource instanceof EntityDamageSourceIndirect) {
                if (((EntityDamageSourceIndirect) damageSource).getProximateDamageSource() instanceof EntityFishingHook) {
                    motX /= 2;
                    motZ /= 2;
                } else if (((EntityDamageSourceIndirect) damageSource).getProximateDamageSource() instanceof EntityArrow) {
                    motX /= 3;
                    motZ /= 3;
                }
            }
    ...
    public BlockFace getOpposite(double rot) {
        if (0 <= rot && rot < 22.5) {
