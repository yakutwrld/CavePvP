...
public class ViewingOtherSharedButtons {

    public static Map<Integer, Button> mainButtons(Player player, PlayerProfile playerProfile, Profile profile, MenuType menuType) {
        ...
        toReturn.put(6, new Button() {
            ...
            public void clicked(Player player, int slot, ClickType clickType) {
                if (clickType == ClickType.SHIFT_RIGHT) {
                    ...
                    player.sendMessage(ChatColor.RED + "Removed " + targetProfile.getName() + ChatColor.RED + " as a friend.");

                    playerProfile.getFriends().remove(player.getUniqueId());
                    playerProfile.save();

                    targetProfile.getFriends().remove(targetProfile.getUuid());
                    targetProfile.save();
                    return;
                }
