...
@AllArgsConstructor
public class EditScoreboardNameButton extends Button {
    private Menu menu;
    private KeyAll keyAll;

    @Override
    public String getName(Player player) {
        return ChatColor.GOLD + ChatColor.BOLD.toString() + "Scoreboard Display Name";
    }

    @Override
    public List<String> getDescription(Player player) {
        return Arrays.asList("", ChatColor.translate("&6&l┃ &fCurrent: &e" + keyAll.getDisplayName()),
                "", ChatColor.GREEN + "Click to modify this key-all's scoreboard name");
    }
    ...
    public static class DisplayNamePrompt extends StringPrompt {
        ...
        public String getPromptText(ConversationContext conversationContext) {
            return ChatColor.YELLOW + "Please type the new display name for the scoreboard keyall. Type " + ChatColor.RED + "cancel" + ChatColor.YELLOW + " to cancel.";
        }
        ...
        public Prompt acceptInput(ConversationContext conversationContext, String input) {
            ...
            if (input.equalsIgnoreCase("cancel")) {
                ...
                conversationContext.getForWhom().sendRawMessage(ChatColor.RED + "Key-All scoreboard display name process cancelled.");
                return END_OF_CONVERSATION;
            }

            menu.openMenu(sender);
            keyAll.setScoreboardDisplay(input);
            Foxtrot.getInstance().getKeyAllHandler().getCache().replace(input, keyAll);

            conversationContext.getForWhom().sendRawMessage(ChatColor.translate("&6Set the scoreboard display-name for &f" + keyAll.getId() + " &6to &f" + input));
