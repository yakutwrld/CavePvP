...
@AllArgsConstructor
public class EditGiveAllButton extends Button {
    private Menu parentMenu;
    private KeyAll keyAll;

    @Override
    public String getName(Player player) {
        return ChatColor.GOLD + ChatColor.BOLD.toString() + "Start Date";
    }

    @Override
    public List<String> getDescription(Player player) {
        return Arrays.asList("", ChatColor.translate("&6&l┃ &fCurrent: &e" + new Date(keyAll.getGiveAllTime()).toLocaleString())
                , "", ChatColor.GREEN + "Click to modify this key-all's start date.");
    }
    ...
    public static class DatePrompt extends StringPrompt {
        ...
        public String getPromptText(ConversationContext conversationContext) {
            return ChatColor.YELLOW + "Please type the start duration. Type " + ChatColor.RED + "cancel" + ChatColor.YELLOW + " to cancel.";
        }
        ...
        public Prompt acceptInput(ConversationContext conversationContext, String input) {
            ...
            keyAll.setGiveAllTime(System.currentTimeMillis()+durationWrapper.getDuration());
            Foxtrot.getInstance().getKeyAllHandler().getCache().replace(input, keyAll);

            conversationContext.getForWhom().sendRawMessage(ChatColor.translate("&6Set the start date for &f" + keyAll.getId() + " &6to &f" + new Date(keyAll.getGiveAllTime()).toLocaleString()));
