...
public class PortableJumpBoost extends Ability {
    @Override
    ...
    public String getDescription() {
        return "Your team has been given JumpBoost VII for 5 seconds.";
    }

    public static ItemStack itemStack;

    public PortableJumpBoost() {
    ...
    public String getDisplayName() {
        return ChatColor.GOLD.toString() + ChatColor.BOLD + "Jump Boost";
    }
    ...
    public List<String> getLore() {
        ...
        toReturn.add(ChatColor.translate("&6❙ &e&l5 seconds &fof &a&lJump Boost"));
        toReturn.add("");
    ...
    public void onInteract(PlayerInteractEvent event) {
        ...
        if (team != null) {
            player.getNearbyEntities(25,25,25).stream().filter(it -> it instanceof Player && team.getOnlineMembers().contains(it))
                    .forEach(it -> ((Player) it).addPotionEffect(new PotionEffect(PotionEffectType.JUMP, 6*20, 6), true));
        }
        player.addPotionEffect(new PotionEffect(PotionEffectType.JUMP, 6*20, 6), true);
