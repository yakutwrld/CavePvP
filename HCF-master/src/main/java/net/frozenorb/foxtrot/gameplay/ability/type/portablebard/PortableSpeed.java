...
public class PortableSpeed extends Ability {
    @Override
    ...
    public String getDescription() {
        return "Your team has been given Speed III for 5 seconds.";
    }

    public static ItemStack itemStack;

    public PortableSpeed() {
    ...
    public String getDisplayName() {
        return ChatColor.GOLD.toString() + ChatColor.BOLD + "Speed";
    }
    ...
    public List<String> getLore() {
        ...
        toReturn.add(ChatColor.translate("&6❙ &e&l5 seconds &fof &b&lSpeed"));
        toReturn.add("");
    ...
    public void onInteract(PlayerInteractEvent event) {
        ...
        if (team != null) {
            player.getNearbyEntities(25,25,25).stream().filter(it -> it instanceof Player && team.getOnlineMembers().contains(it))
                    .forEach(it -> ((Player) it).addPotionEffect(new PotionEffect(PotionEffectType.SPEED, 6*20, 2), true));
        }
        player.addPotionEffect(new PotionEffect(PotionEffectType.SPEED, 6*20, 2), true);
