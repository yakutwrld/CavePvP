...
public class PortableStrength extends Ability {
    ...
    public String getDescription() {
        return "Your team has been given Strength II for 5 seconds.";
    }
    ...
    public List<String> getLore() {
        ...
        toReturn.add(ChatColor.translate("&6❙ &e&l5 seconds &fof &c&lStrength"));
        toReturn.add("");
    ...
    public void onInteract(PlayerInteractEvent event) {
        ...
        if (team != null) {
            player.getNearbyEntities(25,25,25).stream().filter(it -> it instanceof Player && team.getOnlineMembers().contains(it))
                    .forEach(it -> ((Player) it).addPotionEffect(new PotionEffect(PotionEffectType.INCREASE_DAMAGE, 6*20, 1), true));
        }
